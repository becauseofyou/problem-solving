...
const char* latestFeatures[] = {
                          ...
                          "Print checker message to the stdout instead of stdout",  
...
#define WA_EXIT_CODE 3
#define PE_EXIT_CODE 3
#define POINTS_EXIT_CODE 1
#define FAIL_EXIT_CODE 8
#define DIRT_EXIT_CODE WA_EXIT_CODE
...
NORETURN void halt(int exitCode)
{
    ...
    std::fprintf(stdout, "Checker: \"%s\"\n", checkerName.c_str());
    std::fprintf(stdout, "Exit code: %d\n", exitCode);
    InStream::textColor(InStream::LightGray);
#endif
    std::exit(exitCode);
}
...
if ((result)!=_ok && result!=_points) quitscr(LightGray,"0 ");
 ...
 std::fflush(stdout);
   ...
   /* quitscr(LightGray, msg);
    ...
    if (resultName != "")
        std::fprintf(stdout, "See file to check exit message\n");
    ...
    if (!stdfile)
        if (NULL == (file = std::fopen(name.c_str(), "rb")))
        {
            if (mode == _output)
				...
				quits(_pe, std::string("File not found"));
        }
...
NORETURN void __testlib_help()
{
    ...
    std::fprintf(stdout, "TESTLIB %s, http://code.google.com/p/testlib/ ", VERSION);
    std::fprintf(stdout, "by Mike Mirzayanov, copyright(c) 2005-2013\n");
    std::fprintf(stdout, "Checker name: \"%s\"\n", checkerName.c_str());
    InStream::textColor(InStream::LightGray);

    std::fprintf(stdout, "\n");
    std::fprintf(stdout, "Latest features: \n");
    ...
    std::fprintf(stdout, "Program must be run with the following arguments: \n");
    std::fprintf(stdout, "    <input-file> <output-file> <answer-file> [<report-file> [<-appes>]]\n\n");
...
void registerGen(int argc, char* argv[])
{
    std::fprintf(stdout, "Use registerGen(argc, argv, 0) or registerGen(argc, argv, 1)."
            " The third parameter stands for the random generator version."
            " If you are trying to compile old generator use macro -DUSE_RND_AS_BEFORE_087 or registerGen(argc, argv, 0)."
            " Version 1 has been released on Spring, 2013. Use it to write new generators.\n\n");
    registerGen(argc, argv, 0);
...
{
    const std::string testFileName = "data" + vtos(test) + ".in";
    std::cerr <<"Writing data into file: " << testFileName << std::endl;
